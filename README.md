# JDK Version Manager (JVMS) for Windows（Windows的 JDK 版本管理器 ）
...
首先，这个版本的 jvms 不依赖于其他库。它是用[Go](http://golang.org/)编写的，与绕过有限文件进行黑客攻击相比，这是一种更加结构化的方法 。它不依赖于现有的 jdk 安装。`.bat`

控制机制也大不相同。有两种通用的方式来支持具有热切换功能的多个 jdk 安装。 第一种是在任何时候切换版本时修改系统，或者通过使用文件模仿 jdk 可执行文件并相应地重定向来绕过它。这对我来说总是有点老套，而且这个实现有一些怪癖。`PATH``.bat`
...
为了创建/修改符号链接，您必须以管理员身份运行，并且必须绕过 Windows UAC（那个烦人的提示）。因此，JVMS for Windows会维护一个符号链接，该符号链接仅 在运行期间放入系统中。切换到不同版本的 JDK 就是切换符号链接目标。因此，该实用程序不需要**您**在每次打开控制台窗口时都运行。当您运行时*，* jdk 的活动版本会在所有打开的控制台窗口中自动更新。它还会在系统重启之间持续存在，因此您只需在想要进行更改时使用 jvms。`PATH``jvms init` `jvms switch x.x.x` `jvms switch x.x.x`

总的来说，这个项目汇集了一些想法、一些久经沙场的其他模块以及对新版本 JDK 的支持。
...
- 添加你的jdk下载链接。格式是这样的：
  ...
  ```
      “网址”：“http://192.168.1.101/files/jdk/1.9.0.zip”
  ```
